...
脚本采用**规则提取 + 免费机器翻译**的两步流程，全程零成本：

1. **句子提取**：从英文摘要中取前2句（背景/方法）+ 最后1句（结论，若总句数 ≥ 4）
2. **中文翻译**：调用 [MyMemory](https://mymemory.translated.net/) 免费翻译 API（无需 API Key，基于 `urllib` 标准库），将提取内容翻译为简体中文
3. **容错机制**：翻译失败时自动重试2次；全部失败则保留原英文并附注"翻译服务暂不可用"
